...
public:
   ...
   */
  FloatArray(float *data, const size_t numBytes, bool copyData);
   ...
   */
  FloatArray(JNIEnv *env, jfloatArray data);
   ...
   */
  const float* get() const { return _data; }

  float getValByIndex(int iIndex) const { return _data[iIndex]; }

  /**
   * @brief Convert data to a Java byte[] array
   */
  JniLocalRef<jfloatArray> toJavaFloatArray(JNIEnv *env) const;
   ...
   * described above.
   */
  void set(float *data, const size_t numBytes, bool copyData);
   ...
   */
  void set(JNIEnv *env, jfloatArray data);
