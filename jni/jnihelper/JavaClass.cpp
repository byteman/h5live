...
jobject JavaClass::toJavaObject(JNIEnv *env, jobject javaThis) {
  for (FieldMap::const_iterator iter = _fields->begin(); iter != _fields->end(); ++iter) {
    ...
    jfieldID field = iter->second;
    const FieldMapping *mapping = getFieldMapping(key.c_str());
	 LOG_DEBUG("field=%x,mapping=%x", field,mapping);
    if (field != NULL && mapping != NULL) {
      if (TYPE_EQUALS(mapping->type, kTypeInt)) {
        int *address = static_cast<int*>(mapping->address);
        env->SetIntField(javaThis, field, *address);
		LOG_DEBUG("setIntField");
      } else if (TYPE_EQUALS(mapping->type, kTypeShort)) {
        short *address = static_cast<short*>(mapping->address);
      ...
      } else if (TYPE_EQUALS(mapping->type, kTypeArray(kTypeFloat))) {
			
        	//øΩ±¥c++vector µΩjava∂‘œÛ
			FloatArray* pArray = (static_cast<FloatArray*>(mapping->address));
			if( (pArray != NULL) && (pArray->size() > 0) )
				env->SetObjectField(javaThis, field, pArray->toJavaFloatArray(env));
      }
	  else if (TYPE_EQUALS(mapping->type, kTypeArray(kTypeByte))) {
	  	  ByteArray* pArray = (static_cast<ByteArray*>(mapping->address));
		  if( (pArray != NULL) && (pArray->size() > 0) )
		  	env->SetObjectField(javaThis, field, pArray->toJavaByteArray(env));		 
	  }
