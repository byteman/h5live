...
int  ByteArray::copy(JNIEnv *env,jbyteArray in_data)
{
	int size = 0;
	if(in_data == NULL) return 0;
	
	
	size = env->GetArrayLength(in_data);
	if(size < _num_bytes) return 0;
	
	env->SetByteArrayRegion(in_data, 0, (jsize)_num_bytes, (jbyte *)_data);
